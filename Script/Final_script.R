...
Running_MCMCglmm <- function(df, trait_list, value_checking, list_transform, tree){
  ...
  for (k in 1:length(trait_list)){
    ...
    size_lm <- summary(lm(dfbis$transform ~ dfbis$size))
    ...
    if (abs(MoranI$observed) >= 0.1){
      ...
      if (spatial_size_correction[k,1]=="YES"){
        ...
        spatial_size_lm <- gam((dfbis$transform) ~ dfbis$size + te(dfbis$Lon, dfbis$Lat))
    ...
    if (spatial_size_correction[k,1]=="YES"){
      ...
      pglm1 <- MCMCglmm(transform ~ coord1dim + coord2dim + size, random=~phylo+SpeciesAccepted,
                        ginverse = list(phylo = inverseA(treebis,nodes = "TIPS")$Ainv),
                        data=dfbis,
                        prior = prior2,
                        nitt = 25000,
...
dfanimal$size <- log(dfanimal$size)
dfplant$size <- log(dfplant$size)
...
for (i in 1:length(dfanimal$SpeciesAccepted)){
  dfanimal$migration[i] <- animal_migration$Distance[animal_migration$ï..SpeciesAccepted==dfanimal$SpeciesAccepted[i]]
}
...
for (i in 1:length(df$SpeciesAccepted)){
  ...
  if (is.na(df$FirstStepAtt[i]) == FALSE){
    if (df$FirstStepAtt[i] == 0){
      df$resistance_FSA[i] <- log(1/0.000001)
    } else {
      df$resistance_FSA[i] <- log(1/df$FirstStepAtt[i] + 0.000001)
    }
  } else {
    df$resistance_FSA[i] <- NA
  }
  if (is.na(df$MaxAttenuation[i]) == FALSE){
    if (df$MaxAttenuation[i] == 0){
      df$resistance_MA[i] <- log(1/0.000001)
    } else {
      df$resistance_MA[i] <- log(1/df$MaxAttenuation[i] + 0.000001)
    }
  } else {
    df$resistance_MA[i] <- NA
  }
  if (is.na(df$InertiaLow[i]) == FALSE){
    if (df$InertiaLow[i] == 0){
      df$resistance_IL[i] <- log(1/0.000001)
    } else {
      df$resistance_IL[i] <- log(1/df$InertiaLow[i] + 0.000001)
    }
  } else {
...
for (i in 1:length(dfanimal$SpeciesAccepted)){
  ...
  if (is.na(dfanimal$FirstStepAtt[i]) == FALSE){
    if (dfanimal$FirstStepAtt[i] == 0){
      dfanimal$resistance_FSA[i] <- log(1/0.000001)
    } else {
      dfanimal$resistance_FSA[i] <- log(1/dfanimal$FirstStepAtt[i] + 0.000001)
    }
  } else {
    dfanimal$resistance_FSA[i] <- NA
  }
  if (is.na(dfanimal$MaxAttenuation[i]) == FALSE){
    if (dfanimal$MaxAttenuation[i] == 0){
      dfanimal$resistance_MA[i] <- log(1/0.000001)
    } else {
      dfanimal$resistance_MA[i] <- log(1/dfanimal$MaxAttenuation[i] + 0.000001)
    }
  } else {
    dfanimal$resistance_MA[i] <- NA
  }
  if (is.na(dfanimal$InertiaLow[i]) == FALSE){
    if (dfanimal$InertiaLow[i] == 0){
      dfanimal$resistance_IL[i] <- log(1/0.000001)
    } else {
      dfanimal$resistance_IL[i] <- log(1/dfanimal$InertiaLow[i] + 0.000001)
    }
  } else {
...
for (i in 1:length(dfplant$SpeciesAccepted)){
  ...
  if (is.na(dfplant$FirstStepAtt[i]) == FALSE){
    if (dfplant$FirstStepAtt[i] == 0){
      dfplant$resistance_FSA[i] <- log(1/0.000001)
    } else {
      dfplant$resistance_FSA[i] <- log(1/dfplant$FirstStepAtt[i] + 0.000001)
    }
  } else {
    dfplant$resistance_FSA[i] <- NA
  }
  if (is.na(dfplant$MaxAttenuation[i]) == FALSE){
    if (dfplant$MaxAttenuation[i] == 0){
      dfplant$resistance_MA[i] <- log(1/0.000001)
    } else {
      dfplant$resistance_MA[i] <- log(1/dfplant$MaxAttenuation[i] + 0.000001)
    }
  } else {
    dfplant$resistance_MA[i] <- NA
  }
  if (is.na(dfplant$InertiaLow[i]) == FALSE){
    if (dfplant$InertiaLow[i] == 0){
      dfplant$resistance_IL[i] <- log(1/0.000001)
    } else {
      dfplant$resistance_IL[i] <- log(1/dfplant$InertiaLow[i] + 0.000001)
    }
  } else {
...
set.seed(446)
for(i in 1:1000){
  newpc_unif <- (apply(pcs, 2, function(x) runif(nrow(pcs), min = min(x), max = max(x))))
...
set.seed(447)
for(i in 1:1000){
  newpc1 <- matrix(runif(nrow(pcs), min = min(pcs[,1]), max = max(pcs[,1])))
...
test.norm <- as.randtest(obs=obs.vol,sim=sim.norm.vol, alter="less")
